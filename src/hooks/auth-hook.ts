...
const [token, setToken] = useState(false as any);
  ...
  const login = useCallback((username: string, token: string, expirationDate: Date | undefined) => {
    setToken(token);
    ...
    localStorage.setItem(
      "userData",
      JSON.stringify({ token: token, username: username, expiration: tokenExpirationDate.toISOString() })
    );
  }, []);
